...
def  main():
  ...
  installer = "\installer\PitchAnalyzer Installer.exe"
   
  if demo:
    installer = "\installer\PitchAnalyzer Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\PitchAnalyzer manual.pdf" 
  ]
  ...
  ZIPNAME = "PitchAnalyzer-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "PitchAnalyzer-v" + FULLVERSIONSTR + "-win-demo.zip"
